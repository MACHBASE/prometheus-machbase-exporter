...
def run_scheduler(scheduler, machbase_client, name, interval, query, value_columns):
    def scheduled_run(scheduled_time):
        all_metrics = []

        try:
            if machbase_client.execute(query) is 0:
                result = json.loads(machbase_client.result())
                raise Exception(result['MACHBASE_ERROR'])
            
            response = json.loads('[' + machbase_client.result() + ']')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Export Machbase query results to Prometheus.')
    parser.add_argument('-p', '--port', type=int, default=9297,
        help='port to serve the metrics endpoint on. (default: 9297)')
    ...
    parser.add_argument('-s', '--machbase-server', type=server_address, default='localhost',
        help='address of a Machbase server to run queries on. A port can be provided if non-standard (5656) e.g. localhost:6500 (default: localhost)')
    parser.add_argument('-u', '--machbase-user', default='SYS',
        help='Machbase user to run queries as. (default: SYS)')
    parser.add_argument('-P', '--machbase-password', default='',
        help='password for the Machbase user, if required. (default: no password)')
    ...
    port = args.port
    machbase_host, machbase_port = args.machbase_server

    username = args.machbase_user
    password = args.machbase_password
    ...
    for name, (interval, query, value_columns) in queries.items():
        machbase_client = machbase()
        if machbase_client.open(machbase_host, username, password, machbase_port) is 0:
            msg = 'failed to connect to {}:{} ({})'.format(machbase_host, machbase_port,
                                                           machbase_client.result())
            raise Exception(msg)

        run_scheduler(scheduler, machbase_client, name, interval, query, value_columns)
